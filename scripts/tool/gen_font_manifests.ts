    ...
    for (const repo of target_repos) {
        try {
            ...
            const assets = latestRelease!.releaseAssets!.nodes!.filter(
                ...
                return asset.name.toUpperCase().includes(
            ...
            for (const asset of assets) {
                ...
                const manifest_url = resolve(
                    `${bucketDirPath}${manifest_name}.json`,
                );
                ...
                Deno.writeTextFileSync(
                    manifest_url,
                    JSON.stringify(manifest, null, 4) + "\r\n", // Excavatorで更新された時にインデントがspace*4になるのでそれと合わせる
                );
            }
            console.log(
